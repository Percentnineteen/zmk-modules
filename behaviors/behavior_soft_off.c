...
static int on_keymap_binding_released(struct zmk_behavior_binding *binding,
                                      struct zmk_behavior_binding_event event) {
#if !defined(CONFIG_ZMK_DISABLE_DONGLE_SOFT_OFF) || !CONFIG_ZMK_DISABLE_DONGLE_SOFT_OFF
    const struct device *dev = zmk_behavior_get_binding(binding->behavior_dev);
    struct behavior_soft_off_data *data = dev->data;
    const struct behavior_soft_off_config *config = dev->config;
...
#endif // !defined(CONFIG_ZMK_DISABLE_DONGLE_SOFT_OFF) || !CONFIG_ZMK_DISABLE_DONGLE_SOFT_OFF
